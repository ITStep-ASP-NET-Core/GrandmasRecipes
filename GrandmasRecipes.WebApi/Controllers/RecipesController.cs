...
namespace GrandmasRecipes.WebApi.Controllers
{
	...
	public class RecipesController : ControllerBase
	{
		...
		/// <summary>
		/// Отримати рецепти з фільтрацією та пошуком за назвою.
		/// </summary>
		/// <remarks>
		/// Всі параметри фільтра опціональні та можуть комбінуватись між собою.
		/// Підтримується текстовий пошук за назвою (<c>SearchQuery</c>) — регістронезалежний, часткове співпадіння.
		/// Підтримується фільтрація за категоріями, кухнями, рівнями складності та продуктами.
		/// Пошук і фільтри працюють разом — всі умови застосовуються одночасно (AND).
		/// Результат посортований за кількістю лайків (спадання).
		/// </remarks>
		/// <param name="filter">
		/// Параметри фільтрації та пошуку. Дивись <see cref="RecipeFilterDto"/>.
		/// Приклад: <c>?filter.searchQuery=борщ&amp;filter.cuisineIds=1&amp;filter.difficultyIds=2</c>
		/// </param>
		/// <param name="page">Номер сторінки, починається з 1.</param>
		/// <param name="userId">Опціональний ідентифікатор поточного користувача для визначення <c>IsLiked</c>.</param>
