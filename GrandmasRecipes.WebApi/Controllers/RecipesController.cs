...
namespace GrandmasRecipes.WebApi.Controllers
{
    ...
    public class RecipesController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> GetRecipes([FromQuery] int page = 1)
        {
            var result = await _recipeService.GetRecipesAsync(page);
            return Ok(result);
        }

        [HttpGet("filter")]
        public async Task<IActionResult> GetRecipesByFilters([FromQuery] RecipeFilterDto filter, [FromQuery] int page = 1)
        {
            var result = await _recipeService.GetRecipesByFiltersAsync(filter, page);
            return Ok(result);
        }

        [HttpGet("author/{authorId:guid}")]
        public async Task<IActionResult> GetRecipesByAuthor(Guid authorId, [FromQuery] int page = 1)
        {
            var result = await _recipeService.GetRecipesByAuthorAsync(authorId, page);
            return Ok(result);
        }
